...
class MangaDetailScreen extends ConsumerStatefulWidget {
  const MangaDetailScreen({super.key, required this.mediaId});
...
class _MangaDetailScreenState extends ConsumerState<MangaDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final mediaAsync = ref.watch(mangaDetailProvider(widget.mediaId));
    ...
    return Scaffold(
      ...
      body: mediaAsync.when(
        data: (media) => _MangaDetailBody(media: media),
  ...
  Future<void> _openUpdateDrawer(Map<String, dynamic> media) async {
    ...
    final maxByChapters = (media['chapters'] as num?)?.toInt();
    final maxByEpisodes = (media['episodes'] as num?)?.toInt();
    final maxProgress = ((maxByChapters != null && maxByChapters > 0)
            ? maxByChapters
            : maxByEpisodes) ??
        9999;
  ...
  Future<void> _saveListEntry({
    ...
    try {
      ...
      ref.invalidate(mangaDetailProvider(widget.mediaId));
...
class _MangaDetailBody extends StatelessWidget {
  const _MangaDetailBody({required this.media});
